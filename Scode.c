...
trisa = 0b00000011;
porta=0;
T1CON = 0B00000001;  // COUNT EVERY 1 uS..
TMR1L = 0;   TMR1H = 0;
TRISC=0B10111000;
portc=0;
trisd=0b11110000;
...
if (H=='D'){
   lcd_cmd(_lcd_clear);
    ...
    RC0_BIT = 0;
   //END OF TRIG PULSE...
   TMR1L = 0;   TMR1H = 0; //RESET FOR THE TIMER (ALL values = 0 )...
    while( RC3_BIT == 0);
    //START OF ECO ...
    TMR1ON_BIT = 1;   // timer one on!!!
    while( RC3_BIT == 1);
    TMR1ON_BIT =0;
    TIME = TMR1H ;
    TIME = TIME << 8 ;
    TIME = TIME | TMR1L ;
    DISTANCE = 0.01655 * TIME ; // DISTANCE =   ([(331)* TIME * 10^-6 * 100] /2 )   IN CM
    if(DISTANCE<0){
    DISTANCE=0;
    }
